...
public class Archive {
    ...
    private void loadConfigDefaults() {
        ...
        if (!ConfigFile.contains("inf-pages")) {setInf_pages(false); changed = true;}
        if (!ConfigFile.contains("allow-submissions")) {setAllow_submission(false); changed = true;}
        if (!ConfigFile.contains("review-items")) {setReview_items(true); changed = true;}
        if (!ConfigFile.contains("max-stack-size")) {setMax_stack_size(1); changed = true;}
        if (!ConfigFile.contains("max-submission-size")) {setMax_item_size(1000000); changed = true;}
        if (!ConfigFile.contains("min-submission-size")) {setMin_item_size(100); changed = true;}
        if (!ConfigFile.contains("submit-delay")) {setSubmit_delay(10); changed = true;}
        if (!ConfigFile.contains("editors")) { this.editors = new ArrayList<>(); addEditor("_txsla"); changed = true; }
        if (!ConfigFile.contains("submit-bans")) { this.submit_bans = new ArrayList<>(); addSubmit_ban("banned_player"); changed = true;}
        if (!ConfigFile.contains("placeholders") || ConfigFile.getSection("placeholders") == null) {
